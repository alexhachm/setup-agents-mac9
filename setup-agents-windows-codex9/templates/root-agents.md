...
`CODEX_EXPERIMENTAL_AGENT_TEAMS=1` is enabled only for Master-2, Master-3, and workers via generated launchers.
...
`.codex/state/` is a shared link to `.codex-shared-state/` (junction on Windows, symlink on Unix). Always use the lock helper:
```bash
