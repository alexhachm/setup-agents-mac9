...
Logging:
```bash
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [master-3] [TEAM_BURST] id=[request_id] purpose=\"[reason]\" teammates=[N]" >> .codex/logs/activity.log
```
...
3. Capture pre-assignment status, then update worker-status.json with the task assignment
4. **Launch or signal the worker:**
```bash
prev_worker_status=$(jq -r '.["worker-N"].status' .codex/state/worker-status.json)

if [ "$prev_worker_status" = "idle" ]; then
...
6. Capture each worker's pre-assignment status, then update worker-status.json (use lock helper)
...
8. Touch `.codex/signals/.handoff-signal` (so Master-2 can track)
