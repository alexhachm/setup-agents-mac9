...
config_file="$HOME/.codex-multi-agent-config"
...
# Resolve project directory (script lives at .codex/scripts/)
PROJECT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
WORKTREE="$PROJECT_DIR/.worktrees/wt-$WORKER_NUM"
LAUNCHER_PS1="$PROJECT_DIR/.codex/launchers/worker-${WORKER_NUM}.ps1"
LAUNCHER_BAT="$PROJECT_DIR/.codex/launchers/worker-${WORKER_NUM}.bat"
...
#!/usr/bin/env bash
# Stop notification: notifies user when Codex stops execution.
# Windows: PowerShell beep fallback. macOS: system notification. Other: silent.
if [ "$(uname -s)" = "Darwin" ]; then
    osascript -e 'display notification "Done" with title "Codex" sound name "Glass"' 2>/dev/null || true
