...
function renderHealthCards(health) {
    ...
    if (health['master-2']) {
        ...
        html += renderAgentCard('MASTER-2', 'Deep', 'Architect', m2.status || 'active', m2.resetImminent, [
    ...
    if (health['master-3']) {
        ...
        html += renderAgentCard('MASTER-3', 'Deep', 'Allocator', m3.status || 'active', m3.resetImminent, [
    ...
    if (health.workers) {
        Object.entries(health.workers).sort(([a], [b]) => a.localeCompare(b)).forEach(([id, w]) => {
            ...
            html += renderAgentCard(id.toUpperCase(), 'Deep', w.domain || 'general', isDead ? 'dead' : w.status || 'idle', w.resetImminent, [
...
function renderKnowledgeList() {
    ...
    if (state.knowledgeFiles.length === 0) {
        fl.innerHTML = '<div class="knowledge-empty">No knowledge files found in .codex/knowledge/</div>';
        return;
    }
