...
# Contains: root-agents.md, worker-agents.md, subagents, settings.json,
#           knowledge templates, and state templates
# These go into the templates/ directory alongside setup.sh
#
# v3 changes vs v8:
#   - Restored escalation paths section in root-agents.md (from v7)
#   - Restored "Lessons Learned" append section in root-agents.md (from v7)
...
#   - Restored task protocol quick-reference in worker-agents.md (from v7)
#   - Restored emergency commands in worker-agents.md (from v7)
#   - Restored self-check instructions in worker-agents.md (from v7)
...
```
User → Master-1 (Fast) → handoff.json + .handoff-signal
         ↕ clarification-queue.json
       Master-2 (Deep) ─── TRIAGE:
         ...
         └─ Tier 3: Decompose → task-queue.json + .task-signal
                      ↓
       Master-3 (Fast) → `bash .codex/scripts/task-api.sh create --assigned-to worker-N` + .worker-signal
         ↕ worker-status.json
       Workers 1-8 (Deep, isolated worktrees, one domain each)
...
- Master-1: `.codex/docs/master-1-role.md`
- Master-2: `.codex/docs/master-2-role.md`
- Master-3: `.codex/docs/master-3-role.md`
...
`.codex/state/` is a symlink to `.codex-shared-state/`. Always use the lock helper:
```bash
bash .codex/scripts/state-lock.sh .codex/state/<file> '<write command>'
```
...
|--------|--------|---------|---------|
| `.codex/signals/.handoff-signal` | Master-1 | Master-2 | New request ready |
| `.codex/signals/.task-signal` | Master-2 | Master-3 | Decomposed tasks ready |
| `.codex/signals/.worker-signal` | Master-2/3 | Workers | Task assigned |
| `.codex/signals/.fix-signal` | Master-1 | Master-3 | Fix request ready |
| `.codex/signals/.completion-signal` | Workers | Master-3 | Task completed |

**To signal:**
```bash
touch .codex/signals/.signal-name
```

**To wait for signal (with timeout):**
```bash
bash .codex/scripts/signal-wait.sh .codex/signals/.signal-name 30
```
...
**All agents MUST log significant actions** to `.codex/logs/activity.log`:
```bash
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [AGENT_ID] [ACTION] details" >> .codex/logs/activity.log
```
...
| Tier 2 | build-validator only | Economy subagent. No verify-app. |
...
# ==============================================================================
# FILE: templates/worker-agents.md
# ==============================================================================
...
1. Poll `bash .codex/scripts/task-api.sh list` for tasks with `ASSIGNED_TO: worker-N` (your ID)
2. Validate domain (first task sets it; mismatches = error, skip)
3. Claim: `bash .codex/scripts/task-api.sh claim --id [task-id] --owner worker-N`
...
10. Complete: `bash .codex/scripts/task-api.sh complete --id [task-id]` + signal Master-3
...
6. Read `.codex/state/worker-lessons.md` — legacy lessons (backward compat)
...

{
  "permissions": {
    "allow": [
      ...
      "Grep(*)", "Glob(*)", "LS(*)",
      "Bash(.codex/scripts/task-api.sh *)", "Bash(.codex/scripts/state-lock.sh *)"
    ],
  ...
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{"type": "command", "command": "bash -c 'bash \"$CODEX_PROJECT_DIR/.codex/hooks/pre-tool-secret-guard.sh\"'"}]
    }],
    "Stop": [{
      "matcher": "",
      "hooks": [{"type": "command", "command": "bash -c 'bash \"$CODEX_PROJECT_DIR/.codex/hooks/stop-notify.sh\"'"}]
    }]
  }
}
