...
4. **Release the claim** (clear claimed_by after task is created):
   ```bash
   bash .claude/scripts/state-lock.sh .claude/state/worker-status.json 'jq ".\"worker-N\".claimed_by = null | .\"worker-N\".status = \"assigned\" | .\"worker-N\".current_task = \"[task subject]\"" .claude/state/worker-status.json > /tmp/ws.json && mv /tmp/ws.json .claude/state/worker-status.json'
   ```
5. Update handoff.json to `"assigned_tier2"`
6. Signal the worker: `touch .claude/signals/.worker-signal`
...
1. Create RESET task: `TaskCreate({ subject: "RESET: Context limit reached", description: "ASSIGNED_TO: worker-N\n\nYour context has reached the reset threshold. Distill knowledge, then run /clear → /worker-loop to restart with a clean context." })`
...
1. Create reset task: `TaskCreate({ subject: "RESET: Domain reassignment needed", description: "ASSIGNED_TO: worker-N\n\nNew domain needed. Distill knowledge, then run /clear → /worker-loop for the new domain." })`
2. Reset their worker-status.json entry: `tasks_completed: 0, domain: null, status: "resetting", context_budget: 0`
