...
ipcMain.handle('run-setup', async (event, { repoUrl, projectPath: pp, workers, sessionMode }) => {
    ...
    if (isWindows) {
        // Try setup.ps1 in same dir, then sibling dirs
        setupScript = path.join(setupScriptDir, 'setup.ps1');
        if (!fs.existsSync(setupScript)) {
            // Check sibling directories (e.g. setup-agents-mac9)
            const parentDir = path.dirname(setupScriptDir);
            const siblings = fs.readdirSync(parentDir, { withFileTypes: true }).filter(d => d.isDirectory());
            for (const sib of siblings) {
                const candidate = path.join(parentDir, sib.name, 'setup.ps1');
